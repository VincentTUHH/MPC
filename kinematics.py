...
class Kinematics:
    ...
    def update(self, q):
        ...
        # Berechne die Transformationen für alle Gelenke
        # TF_i[i] gibt die Transformation von Basis zu Gelenk i an,
        # weil Matrizen bereits multipliziert werden 
        for i in range(self.n_joints + 1):
            self.TF_i[i] = dh2matrix(*self.DH_table[i])
            if i > 0:
                self.TF_i[i] = np.dot(self.TF_i[i-1], self.TF_i[i]) # np.dot for matrix multiplication A @ B
